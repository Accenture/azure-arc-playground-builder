...
az connectedk8s connect --name $clusterid --resource-group $clusterid --custom-locations-oid $myAzureServicePrincipalObjectId
...
az k8s-extension create --name azuremonitor-containers --cluster-type connectedClusters --cluster-name $clusterid --resource-group $clusterid --extension-type Microsoft.AzureMonitor.Containers --configuration-settings logAnalyticsWorkspaceResourceID="/subscriptions/$myAzureSubscriptionId/resourceGroups/$clusterid/providers/Microsoft.OperationalInsights/workspaces/$clusterid"
