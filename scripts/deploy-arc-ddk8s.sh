...
function create_local_storage_provisioner(){
    echo_n_log 'Creating local storage provisioner on local cluster...'
...
echo_n_log "Creating Azure Resource Group ${RESOURCE_GROUP} in ${REGION} region..."
...
echo_n_log "Switching kubectx to ${KUBECTX_FLAG}..."
...
echo_n_log "Connecting Docker Desktop cluster to Azure Arc (this will take a few minutes)..."
...
echo_n_log "Installing app service extensions in dd k8s cluster (this will take a few minutes)..."
APPSERVICE_EXTENSION_ID=$(
    az k8s-extension create \
    -g $RESOURCE_GROUP \
...
echo_n_log "Updating load balancer service with external IP ${LOCAL_HOST_PUBLIC_IP}..."
...
echo_n_log 'Creating persistent volume in ddk8s cluster...'
...
echo_n_log 'Creating persistent volume claim in ddk8s cluster...'
...
echo_n_log "Creating custom location ${CUSTOM_LOCATION_NAME}..."
...
echo_n_log "Creating App Service Kubernetes Environment ${APPSERVICE_KUBE_ENV_NAME} (this may take a few minutes)..."
...
echo_n_log 'Creating App Service plan...'
...
echo_n_log 'Creating App Service...'
...
if [[ "${CREATE_ARC_DATA_SERVICES}" == 'true' ]]; then
    echo_n_log 'Adding PostgreSQL connection string to app service config...'
...
echo_n_log "Deploying Hello World web app to ${APPSERVICE_NAME} in ${CUSTOM_LOCATION_NAME}..."
